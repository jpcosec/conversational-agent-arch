...
El negocio PSP Selfix (Laboratorio Chile / Teva) exige una cadena de atención de 5 etapas (sanitización, clasificador, agentes de dominio, supervisores, compuerta regulatoria) y un árbol de decisión de 4 ramas (journey operativo F0, derivación FV, derivación MedInfo, respuesta IA con policy gate) definidos en /home/jp/AntonIA/projects/teva/PSP//home/jp/AntonIA/projects/teva/PSP/docs/psp-arquitectura-resumen.md y /home/jp/AntonIA/projects/teva/PSP/specs/psp-flujo-atencion-chatbot.yml (proyecto externo, fuera de este repo). El runtime actual absorbe parcialmente los roles (decide_turn=orquestador/supervisor, ContextCompiler+Conversador=agentes de dominio, PII scrubber=sanitización), pero la KB de Antonia solo modela la rama de eventos adversos. La decisión de diseño es KB-first: extender knowledge/ lo máximo posible sin tocar código del runtime, porque el compilador ya entrega toda la KB al LLM por turno y el conocimiento bien redactado cambia el comportamiento sin riesgo de regresión.
...
El modelo GateCriterion existe, importa y está registrado en el store; los ~18 átomos nuevos existen en knowledge/atoms/ con frontmatter tipado válido (4 RuleAtom de clasificación, 4 ConversationStep, 5 GateCriterion, 5 DomainAtom); el store reindexado los expone (SLDBReader.find los recupera por type.knowledge.rule / type.knowledge.step / type.knowledge.domain / type.knowledge.gate); el grafo conversation:steps.* no tiene transiciones colgantes; los átomos gate NO aparecen en el contexto compilado del turno (gate_in_ctx=0 verificado con Vertex+Gemini); y una conversación de prueba por cada una de las 4 ramas PSP con Vertex+Gemini muestra al agente respondiendo acorde a cada intención (operacional: ofrece cambio, EA: reconoce y deriva a médico, MedInfo: deriva a profesional, tratamiento: ofrece video de aplicación) sin ninguna modificación del código de turno (decide_turn, compiler, state_machine, orchestrator). Node navigación (flow_node) no se activa automáticamente — requiere fase de código.
